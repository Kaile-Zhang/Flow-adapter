...
from ip_adapter.attention_processor import IfAttnProcessor2_2
...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        clip_image = self.clip_image_processor(images=orig_file_raw, return_tensors="pt").pixel_values
...
class IPAdapter(torch.nn.Module):
    ...
    def forward(self, noisy_latents, timesteps, encoder_hidden_states, image_embeds, instruct_tokens):
        ip_tokens = self.image_proj_model(image_embeds)
        encoder_hidden_states = torch.cat([encoder_hidden_states, instruct_tokens, ip_tokens], dim=1)
...
def main():
    ...
    for name in unet.attn_processors.keys():
        ...
        if cross_attention_dim is None:
            ...
            weights = {
                ...
                "to_k_in.weight": unet_sd[layer_name + ".to_k.weight"],
                "to_v_in.weight": unet_sd[layer_name + ".to_v.weight"],
            }
            attn_procs[name] = IfAttnProcessor2_2(hidden_size=hidden_size, cross_attention_dim=cross_attention_dim)
    ...
    inference = 0
    if inference:
        for i in ['1.to_k_ip.weight', '1.to_k_in.weight']:
    ...
    for epoch in range(0, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(ip_adapter):
                ...
                noise_pred = ip_adapter(noisy_latents, timesteps, encoder_hidden_states, image_embeds, instruc_tokens)
